...
TEST_CASE("SGEMM Global Memory Coalescing - Basic functionality", "[sgemm_coalesce]") {
    ...
    SECTION("Small square matrices") {
        ...
        auto C = sgemm_global_mem_coalesce(A, B, 1.0f, 0.0f);
    ...
    SECTION("Alpha and beta scaling") {
        ...
        auto C = sgemm_global_mem_coalesce(A, B, alpha, beta, C_init);
...
TEST_CASE("SGEMM Coalescing vs Naive - Correctness comparison", "[sgemm_compare]") {
    ...
    SECTION("Both implementations should produce same results") {
        ...
        auto C_naive = sgemm_naive(A, B, 1.0f, 0.0f);
        auto C_coalesce = sgemm_global_mem_coalesce(A, B, 1.0f, 0.0f);
